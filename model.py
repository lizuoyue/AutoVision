...
def predict_labels_multi_scale(images,
  ...
  for output in sorted(outputs_to_predictions):
    ...
    outputs_to_predictions[output+'_prob'] = predictions
...
def predict_labels(images, model_options, image_pyramid=None):
  ...
  for output in sorted(outputs_to_scales_to_logits):
    ...
    if model_options.prediction_with_upsampled_logits:
      ...
      predictions[output+'_prob'] = tf.nn.softmax(logits, axis=3)
    else:
      ...
      resized_logits = tf.image.resize_nearest_neighbor(
          logits,
          tf.shape(images)[1:3],
          align_corners=True,
          name='resize_logits')
      predictions[output] = tf.squeeze(argmax_results, 3)
      predictions[output+'_prob'] = tf.nn.softmax(resized_logits, axis=3)
